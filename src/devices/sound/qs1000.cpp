		...
		if (chan.m_flags & QS1000_PLAYING)
		{
			if (chan.m_flags & QS1000_ADPCM)
			{
				for (int samp = 0; samp < stream.samples(); samp++)
				{
					...
					stream.add_int(0, samp, result * 8 * lvol * vol, 32768 << 12);   // MAMEFX,  Mamesick 2016-08-31
					stream.add_int(1, samp, result * 8 * rvol * vol, 32768 << 12);   // MAMEFX
				}
			}
			else
			{
				for (int samp = 0; samp < stream.samples(); samp++)
				{
					...
					stream.add_int(0, samp, result * 2 * lvol * vol, 32768 << 12);   // MAMEFX,  Mamesick 2016-08-31
					stream.add_int(1, samp, result * 2 * rvol * vol, 32768 << 12);   // MAMEFX
				}
			}
		}
	}
