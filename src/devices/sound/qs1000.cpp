		...
		if (chan.m_flags & QS1000_PLAYING)
		{
			if (chan.m_flags & QS1000_ADPCM)
			{
				for (int samp = 0; samp < outputs[0].samples(); samp++)
				{
					...
					outputs[0].add_int(samp, result * 8 * lvol * vol, 32768 << 12);   // MAMEFX,  Mamesick 2016-08-31
			...
			else
			{
				for (int samp = 0; samp < outputs[0].samples(); samp++)
				{
					...
					outputs[0].add_int(samp, result * 2 * lvol * vol, 32768 << 12);   // MAMEFX,  Mamesick 2016-08-31
